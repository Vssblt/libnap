...
JsonNode::JsonNode() { _bintype = Number; }

JsonNode::~JsonNode() {}

binstream JsonNode::key() { return _key; }

void JsonNode::key(binstream key) { _key = std::move(key); }

int JsonNode::asInt() const { return _bindata.to<int>(); }

long long JsonNode::asLong() const { return _bindata.to<long long>(); }

double JsonNode::asFloat() const { return _bindata.to<double>(); }

std::string JsonNode::asString() const { return _bindata.toStdString(); }

binstream JsonNode::asBinstream() const { return _bindata; }

bool JsonNode::asBool() const { return _bindata.to<bool>(); }
...
bool JsonNode::asNull() const { return is_null; }

void JsonNode::setNull(bool null) { this->is_null = null; }

////////////////////

JsonNode &JsonParser::root() { return _root; }
...
int JsonParser::getBooleanLen(int pos) {
  if (json.at(pos) == 't') {
    if (json.at(pos + 1ll) == 'r' && json.at(pos + 2ll) == 'u' &&
        json.at(pos + 3ll) == 'e')
      return 4;
  }
  if (json.at(pos) == 'f') {
    if (json.at(pos + 1ll) == 'a' && json.at(pos + 2ll) == 'l' &&
        json.at(pos + 3ll) == 's' && json.at(pos + 4ll) == 'e')
      return 5;
  }
  return -1;
}
...
int JsonParser::getNullLen(int pos) {
  if (json.at(pos) == 'n') {
    if (json.at(pos + 1) == 'u' && json.at(pos + 2) == 'l' &&
        json.at(pos + 3) == 'l')
      return 4;
  }
  return -1;
...
bool JsonParser::parseKV(int &pos, JsonNode &node) {
  // Key-Value requires
  //  a string at the beginning, a colon in the middle, and a legal type at the
  //  end
  SKIP_SPACE(pos, json);
  if (json.at(pos) != D_Q) {
    setError(pos, 3);
