...
void binstream::reserve(size_t len) { _recap(len); }

void binstream::resize(size_t len) { _recap_hold(len); }
...
void binstream::append(const std::string &str) {
  _append((const uint8_t *)(str.c_str()), str.length());
}

void binstream::append(const binstream &str) {
  _append(str.content, str.size());
}
...
void binstream::_append(const uint8_t *con, size_t _length) {
  ...
  if (totallength > capacity) {
    ...
    memcpy(temp_con + static_cast<size_t>(length), con,
